 ...
 return <div className="min-h-screen">
  ...
  <header className="sticky top-0 z-50 border-b border-white/10 bg-[#07111f]/90 backdrop-blur-xl">
   <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-3">
    ...
    <nav className="hidden items-center gap-1 lg:flex">
      <a className="rounded-lg px-3 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white" href={pathFor('/')+'#tools'}>Tools</a>
      <Link className="rounded-lg px-3 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white" to={pathFor('/sources')}>Sources</Link>
      <Link className="rounded-lg px-3 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white" to={pathFor('/methodology')}>Methodology</Link>
      <Link className="rounded-lg px-3 py-2 text-sm text-slate-300 hover:bg-white/5 hover:text-white" to={pathFor('/contact')}>Contact</Link>
    </nav>
  ...
  <footer className="mt-20 border-t border-white/10 bg-slate-950/40 py-12">
   <div className="mx-auto grid max-w-7xl gap-8 px-4 text-sm md:grid-cols-4">
    ...
    <div><p className="font-semibold text-white">Trust & methodology</p><div className="mt-3 grid gap-2 text-slate-400"><Link to={pathFor('/sources')}>Official sources</Link><Link to={pathFor('/methodology')}>Methodology</Link><Link to={pathFor('/privacy')}>Privacy</Link></div></div>
    <div><p className="font-semibold text-white">AnyTool</p><div className="mt-3 grid gap-2 text-slate-400"><Link to={pathFor('/about')}>About</Link><Link to={pathFor('/contact')}>Contact us</Link><Link to={pathFor('/dashboard')}>Account</Link></div></div>
   </div>
