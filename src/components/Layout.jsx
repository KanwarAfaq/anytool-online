...
export default function Layout({children}){
 ...
 const N={
  en:{tools:'Tools',sources:'Sources',contact:'Contact',verified:'2026 Taiwan data verified',search:'Search tools',menu:'Menu',close:'Close',privacy:'Privacy',about:'About',method:'Methodology',footer:'Fast tools. Clear results. Less clutter.',need:'Need a tool?',send:'Tell us →'},
  'zh-TW':{tools:'工具',sources:'來源',contact:'聯絡',verified:'2026 台灣資料已查核',search:'搜尋工具',menu:'選單',close:'關閉',privacy:'隱私權',about:'關於',method:'方法',footer:'快速工具、清楚結果、少一點干擾。',need:'需要其他工具？',send:'告訴我們 →'},
  ar:{tools:'الأدوات',sources:'المصادر',contact:'اتصل',verified:'تم التحقق من بيانات تايوان 2026',search:'ابحث عن أداة',menu:'القائمة',close:'إغلاق',privacy:'الخصوصية',about:'حول',method:'المنهجية',footer:'أدوات سريعة. نتائج واضحة. فوضى أقل.',need:'تحتاج أداة؟',send:'أخبرنا ←'},
  ur:{tools:'ٹولز',sources:'ذرائع',contact:'رابطہ',verified:'2026 تائیوان ڈیٹا تصدیق شدہ',search:'ٹول تلاش کریں',menu:'مینو',close:'بند کریں',privacy:'پرائیویسی',about:'تعارف',method:'طریقۂ کار',footer:'تیز ٹولز۔ واضح نتائج۔ کم شور۔',need:'کوئی ٹول چاہیے؟',send:'ہمیں بتائیں ←'}
 }[lang]
 ...
 return <div className="site-shell min-h-screen">
  ...
  <header className="sticky top-0 z-50 px-3 pt-3">
   <div className="glass mx-auto flex max-w-7xl items-center gap-2 rounded-2xl px-3 py-2.5">
    ...
    <button onClick={()=>setPalette(true)} className="ms-auto hidden min-w-[12rem] max-w-sm flex-1 items-center gap-2 rounded-xl border border-white/[0.08] bg-[#080e16] px-3 py-2.5 text-left text-sm text-slate-500 transition hover:border-lime-300/20 md:flex">
    ...
    <button aria-label={N.search} className="btn-ghost px-3 md:hidden" onClick={()=>setPalette(true)}><Search size={17}/></button>
    ...
    <NavLink className="btn-ghost px-3" to={pathFor('/dashboard')}><UserRound size={17}/></NavLink>
  ...
  {palette&&<div className="command-backdrop" role="dialog" aria-modal="true" aria-label={N.search} onMouseDown={e=>{if(e.target===e.currentTarget)setPalette(false)}}>
    <div className="command-panel reveal">
      <div className="flex items-center gap-3 border-b border-white/[0.07] p-4">
        ...
        <input ref={inputRef} value={q} onChange={e=>setQ(e.target.value)} className="w-full bg-transparent text-base text-white outline-none placeholder:text-slate-600" placeholder={N.search}/>
      ...
      <div className="max-h-[55vh] overflow-auto p-2">{hits.map((tool,i)=><button key={tool.slug} onClick={()=>openTool(tool)} className="command-result flex w-full items-center gap-3 rounded-xl p-3 text-left" data-active={i===0?'true':'false'}>
