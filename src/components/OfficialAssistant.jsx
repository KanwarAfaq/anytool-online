...
export default function OfficialAssistant({slug}){
 ...
 const L={
  en:{title:'Ask the official-source assistant',desc:'Answers are grounded only in current pages from the responsible Taiwan government authorities.',question:'Question for the official-source assistant',placeholder:'Example: Who qualifies, what is the current amount, or what photo rule applies?',ask:'Ask',signin:'Sign in to use the source-backed assistant.',working:'Checking official sources…',note:'AI can still misunderstand text. Open the cited government links for the controlling rule.'},
  'zh-TW':{title:'詢問官方來源助理',desc:'回答僅依據台灣主管機關目前公開頁面。',question:'詢問官方來源助理的問題',placeholder:'例如：誰符合資格？目前補助多少？證件照規格是什麼？',ask:'詢問',signin:'請先登入使用官方來源助理。',working:'正在查核官方來源…',note:'AI 仍可能誤解文字；正式規定請以引用的政府頁面為準。'},
  ar:{title:'اسأل مساعد المصادر الرسمية',desc:'الإجابات مبنية فقط على صفحات الجهات الحكومية التايوانية المسؤولة.',question:'سؤال لمساعد المصادر الرسمية',placeholder:'مثال: من المؤهل؟ ما المبلغ الحالي؟ ما قاعدة الصورة؟',ask:'اسأل',signin:'سجّل الدخول لاستخدام مساعد المصادر.',working:'جارٍ فحص المصادر الرسمية…',note:'قد يسيء AI فهم النص؛ ارجع إلى الروابط الحكومية المذكورة للحكم النهائي.'},
  ur:{title:'سرکاری ماخذ اسسٹنٹ سے پوچھیں',desc:'جوابات صرف متعلقہ تائیوان سرکاری اداروں کے موجودہ صفحات پر مبنی ہیں۔',question:'سرکاری ماخذ اسسٹنٹ کے لیے سوال',placeholder:'مثال: کون اہل ہے؟ موجودہ رقم کیا ہے؟ فوٹو کا کون سا اصول لاگو ہوتا ہے؟',ask:'پوچھیں',signin:'ماخذ پر مبنی اسسٹنٹ استعمال کرنے کے لیے سائن اِن کریں۔',working:'سرکاری ذرائع چیک کیے جا رہے ہیں…',note:'AI متن غلط سمجھ سکتا ہے؛ حتمی اصول کے لیے حوالہ دیے گئے سرکاری لنکس کھولیں۔'}
 }[lang]||null
 ...
 return <section className="mt-7 rounded-2xl border border-sky-300/15 bg-sky-300/[0.035] p-5">
  ...
  <form className="mt-4 flex flex-col gap-3 sm:flex-row" onSubmit={ask}><label className="min-w-0 flex-1"><span className="sr-only">{L.question}</span><input aria-label={L.question} className="input" required maxLength="700" value={question} onChange={e=>setQuestion(e.target.value)} placeholder={L.placeholder}/></label><button className="btn-primary shrink-0"><Send className="me-2" size={15}/>{L.ask}</button></form>
  {status&&<p role="status" aria-live="polite" className="status-message mt-3 text-sm text-amber-200">{status}</p>}
