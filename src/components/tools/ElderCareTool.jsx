...
export default function ElderCareTool(){
  ...
  const totalPublishedOpenBeds=filtered.reduce((sum,row)=>sum+(typeof row.availableBeds==='number'?row.availableBeds:0),0)
  ...
  return <div className="space-y-6">
    ...
    <section className="card p-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="min-w-[240px] flex-1"><label htmlFor="elder-dataset" className="mb-1.5 block text-sm text-slate-400">{L.city}</label><select id="elder-dataset" className="input max-w-xl" value={dataset} onChange={e=>{setDataset(e.target.value);setQuery('')}}><option value="taoyuan">{L.taoyuan}</option><option value="taipei">{L.taipei}</option></select></div>
        <button className="btn-ghost" onClick={()=>load()}><RefreshCw size={15} className="me-2"/>{L.refresh}</button>
      </div>
      <div className="relative mt-4 max-w-xl"><Search size={16} className="absolute start-3 top-3 text-slate-500"/><input aria-label={L.search} className="input ps-9" placeholder={L.search} value={query} onChange={e=>setQuery(e.target.value)}/></div>
      ...
      {!loading&&!error&&<div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-xl bg-white/5 p-4"><div className="text-xs text-slate-500">{L.facilityCount}</div><div className="mt-1 text-2xl font-black">{filtered.length}</div></div><div className="rounded-xl bg-white/5 p-4"><div className="text-xs text-slate-500">{L.openBedTotal}</div><div className="mt-1 text-2xl font-black">{totalPublishedOpenBeds.toLocaleString()}</div></div></div>}
      {loading&&<p role="status" aria-live="polite" className="py-8 text-slate-400">{L.loading}</p>}
      {error&&<div className="py-6" role="alert"><p className="text-amber-300">{error}</p><a className="mt-3 inline-flex text-emerald-300" href={currentSource.url} target="_blank" rel="noreferrer">{L.source} <ExternalLink className="ms-1" size={14}/></a></div>}
