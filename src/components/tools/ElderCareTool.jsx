  ...
  return <div className="space-y-6">
    ...
    <section>
      ...
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
       {[
        ['Taoyuan','taoyuanLocalElderPlacement','NT$10,000 / NT$24,000 per month'],
        ['Taipei','taipeiLocalElderPlacement','NT$4,320–29,250 per month'],
        ['New Taipei','newTaipeiLocalElderPlacement','NT$6,000 / 10,000 / 24,000 per month'],
        ['Taichung','taichungLocalElderPlacement','Up to NT$21,000 + eligible care add-on'],
        ['Tainan','tainanLocalElderPlacement','NT$12,000–25,400 per month'],
       ].map(([city,key,amount])=>{const src=officialSources[key];return <a key={key} href={src.url} target="_blank" rel="noreferrer" className="card p-4 transition hover:border-emerald-400/30"><div className="flex items-start justify-between gap-2"><strong>{city}</strong><ExternalLink size={14} className="mt-1 shrink-0 text-emerald-300"/></div><div className="mt-2 text-lg font-black text-emerald-200">{amount}</div><p className="mt-2 text-xs leading-5 text-slate-400">{src.summary}</p><p className="mt-2 text-[11px] text-slate-500">{src.authority} · {src.verified}</p></a>})}
      </div>
    </section>
    <section className="card p-5">
