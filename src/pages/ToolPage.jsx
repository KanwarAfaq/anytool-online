...
export default function ToolPage(){
 ...
 return <section className="mx-auto max-w-5xl px-4 py-12"><Seo title={toolName(tool)+' | AnyTool.online'} description={toolDescription(tool)} jsonLd={[{'@context':'https://schema.org','@type':'SoftwareApplication',name:toolName(tool),description:toolDescription(tool),applicationCategory:'UtilitiesApplication',operatingSystem:'Web',offers:{'@type':'Offer',price:'0',priceCurrency:'USD'}},{'@context':'https://schema.org','@type':'BreadcrumbList',itemListElement:[{'@type':'ListItem',position:1,name:'AnyTool',item:'https://anytool.online/'},{'@type':'ListItem',position:2,name:toolName(tool)}]}]}/><div className="mb-7"><Link className="text-sm text-emerald-300" to="/">← {t('allTools')}</Link><div className="mt-3 flex items-start justify-between gap-4"><div><h1 className="text-3xl font-black sm:text-4xl">{toolName(tool)}</h1><p className="mt-3 max-w-2xl text-slate-400">{toolDescription(tool)}</p></div><button aria-label={t('favorites')} className="btn-ghost shrink-0" onClick={()=>saveFavorite(slug).then(()=>alert(t('saved'))).catch(()=>alert(t('signInFirst')))}><Heart size={17}/></button></div></div>{view}<ToolGuide tool={tool}/><SourceEvidence slug={slug}/><p className="mt-6 text-xs text-slate-500">{t('planningOnly')}</p></section>
}
