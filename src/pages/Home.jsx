...
export default function Home(){
 ...
 return <>
  ...
  <section className="mx-auto max-w-7xl px-4 py-6">
    <div className="mb-3 flex items-center gap-2 text-xs font-black uppercase tracking-[.16em] text-slate-600"><Zap size={13} className="text-lime-300"/>{C.ticker}</div>
  ...
  {recent.length>0&&<section className="mx-auto max-w-7xl px-4 py-7 scroll-reveal">
    <div className="mb-3 text-xs font-black uppercase tracking-[.16em] text-slate-600">{C.recent}</div>
    <div className="scrollbar-none flex gap-3 overflow-x-auto pb-2">{recent.map(tool=>{const I=categoryIcon[tool.category]||Sparkles;return <Link key={tool.slug} to={pathFor('/tools/'+tool.slug)} className={'tool-card category-'+tool.category+' min-w-[220px] rounded-2xl border border-white/[0.08] bg-[#0d1520] p-4'}><div className="flex items-center gap-3"><span className="tool-icon"><I size={16}/></span><span className="truncate font-bold">{toolName(tool)}</span></div></Link>})}</div>
  </section>}

  <section id="tools" className="mx-auto max-w-7xl px-4 py-10">
   <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
    <div><div className="flex items-center gap-2 text-xs font-black uppercase tracking-[.16em] text-lime-300"><ScanLine size={13}/>{tools.length} tools</div><h2 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">{C.toolsTitle}</h2></div>
   ...
   <div className="scrollbar-none flex gap-2 overflow-x-auto pb-4"><button onClick={()=>setFilter('all')} className={filter==='all'?'btn-primary whitespace-nowrap':'btn-ghost whitespace-nowrap'}>{C.all}</button>{categories.map(c=>{const I=categoryIcon[c.id]||Sparkles;return <button key={c.id} onClick={()=>setFilter(c.id)} className={filter===c.id?'btn-primary whitespace-nowrap':'btn-ghost whitespace-nowrap'}><I className="me-2" size={14}/>{t('categories.'+c.id)}</button>})}</div>
   <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{filtered.map((tool,i)=>{const I=categoryIcon[tool.category]||Sparkles;return <Link to={pathFor('/tools/'+tool.slug)} key={tool.slug} className={'card tool-card category-'+tool.category+' group p-4 scroll-reveal'} style={{animationDelay:Math.min(i,10)*25+'ms'}}><div className="flex items-center gap-3"><span className="tool-icon"><I className={categoryAccent[tool.category]} size={16}/></span><div className="min-w-0 flex-1"><h3 className="truncate font-black tracking-tight group-hover:text-lime-200">{toolName(tool)}</h3><p className="mt-1 line-clamp-1 text-xs text-slate-500">{toolDescription(tool)}</p></div><ArrowRight className="shrink-0 text-slate-700 transition group-hover:translate-x-1 group-hover:text-lime-300" size={15}/></div></Link>})}</div>
