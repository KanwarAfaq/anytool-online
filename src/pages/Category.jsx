...
export default function Category(){
 ...
 return <section className="mx-auto max-w-7xl px-4 py-16"><Seo title={title+' Tools | AnyTool.online'} description={desc} jsonLd={{'@context':'https://schema.org','@type':'CollectionPage',name:title,description:desc}}/><p className="text-xs font-bold uppercase tracking-[.18em] text-emerald-300">AnyTool collection</p><h1 className="mt-2 text-4xl font-black">{title}</h1><p className="mt-4 max-w-3xl text-lg leading-8 text-slate-300">{desc}</p><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{list.map(tool=><Link to={pathFor('/tools/'+tool.slug)} key={tool.slug} className="card group p-5 transition hover:-translate-y-0.5 hover:border-emerald-400/30"><h2 className="font-bold group-hover:text-emerald-300">{toolName(tool)}</h2><p className="mt-2 text-sm leading-6 text-slate-400">{toolDescription(tool)}</p><span className="mt-4 inline-flex items-center text-sm text-emerald-300">{t('open')} <ArrowRight className="ms-1" size={15}/></span></Link>)}</div></section>
}
