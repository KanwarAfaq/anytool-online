...
export default function Dashboard(){
 ...
 if(user===undefined)return <><Seo title="Account Dashboard | AnyTool.online" description="Manage your AnyTool profile, favorites and account access." noindex/><div className="mx-auto max-w-5xl px-4 py-20">{t('loading')}</div></>
 if(!user)return <><Seo title="Account Dashboard | AnyTool.online" description="Manage your AnyTool profile, favorites and account access." noindex/><div className="mx-auto max-w-4xl px-4 py-20"><div className="card p-6"><h1 className="text-2xl font-black">{t('dashboard')}</h1><p className="mt-2 text-slate-400">{t('dashboardDesc')}</p><Link to={pathFor('/auth')} className="btn-primary mt-5">{t('signIn')}</Link></div></div></>
 const name=profile?.display_name||profile?.full_name||user.email?.split('@')[0]||'User'
 return <section className="mx-auto max-w-5xl px-4 py-16"><Seo title="Account Dashboard | AnyTool.online" description="Manage your AnyTool profile, favorites and account access." noindex/><div className="grid gap-5 md:grid-cols-[1fr_280px]">
