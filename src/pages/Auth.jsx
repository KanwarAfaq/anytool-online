...
export default function AuthPage(){
 ...
 return <section className="mx-auto max-w-md px-4 py-16"><Seo title={title+' | AnyTool.online'} description="Secure AnyTool account access with password, email OTP and password recovery." noindex/><div className="card p-6"><h1 className="text-2xl font-black">{title}</h1>
  {(mode==='signin'||mode==='signup'||mode==='reset')&&<form onSubmit={passwordSubmit} className="mt-6 space-y-4">
    {mode!=='reset'&&<input aria-label={t('email')} autoComplete="email" className="input" type="email" required placeholder={t('email')} value={email} onChange={e=>setEmail(e.target.value)}/>}
    <input aria-label={mode==='reset'?L.newPassword:t('password')} autoComplete={mode==='signin'?'current-password':'new-password'} className="input" type="password" required minLength={8} placeholder={mode==='reset'?L.newPassword:t('password')} value={password} onChange={e=>setPassword(e.target.value)}/>
    <button className="btn-primary w-full">{mode==='signin'?t('signIn'):mode==='signup'?t('signUp'):L.reset}</button>
  </form>}
  {mode==='otp'&&<><p className="mt-3 text-sm leading-6 text-slate-400">{L.otpNote}</p><form onSubmit={sendOtp} className="mt-4 flex gap-2"><input aria-label={t('email')} autoComplete="email" className="input" type="email" required placeholder={t('email')} value={email} onChange={e=>setEmail(e.target.value)}/><button className="btn-primary shrink-0">{L.sendOtp}</button></form><form onSubmit={verifyOtp} className="mt-3 flex gap-2"><input aria-label={L.otp} className="input" inputMode="numeric" pattern="[0-9]{6}" placeholder="123456" minLength={6} maxLength={6} required autoComplete="one-time-code" value={otp} onChange={e=>setOtp(e.target.value.replace(/\D/g,'').slice(0,6))}/><button className="btn-ghost shrink-0" disabled={otp.length!==6}>{L.verifyOtp}</button></form></>}
  {mode==='forgot'&&<><p className="mt-3 text-sm text-slate-400">{L.forgotNote}</p><form onSubmit={forgot} className="mt-4 space-y-4"><input aria-label={t('email')} autoComplete="email" className="input" type="email" required placeholder={t('email')} value={email} onChange={e=>setEmail(e.target.value)}/><button className="btn-primary w-full">{L.reset}</button></form></>}
  {msg&&<p role="status" aria-live="polite" className="status-message mt-4 text-sm text-amber-200">{msg}</p>}
