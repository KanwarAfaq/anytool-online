...
export default function Profile(){
 ...
 if(user===undefined)return <div className="mx-auto max-w-4xl px-4 py-20 text-slate-400">Loading…</div>
 if(!user)return <div className="mx-auto max-w-4xl px-4 py-20"><div className="card p-6"><h1 className="text-2xl font-black">Profile</h1><p className="mt-2 text-slate-400">Sign in to edit your profile.</p><Link className="btn-primary mt-5" to={pathFor('/auth')}>Sign in</Link></div></div>
 return <section className="mx-auto max-w-4xl px-4 py-16"><Seo title="Edit Profile | AnyTool.online" description="Manage your AnyTool profile and preferences."/><h1 className="text-3xl font-black">Edit profile</h1><p className="mt-2 text-slate-400">{user.email}</p><form onSubmit={save} className="card mt-7 grid gap-4 p-6 md:grid-cols-2">
