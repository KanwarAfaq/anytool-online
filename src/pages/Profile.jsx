...
export default function Profile(){
 ...
 return <section className="mx-auto max-w-4xl px-4 py-16"><Seo title={L.title+' | AnyTool.online'} description={L.manage} noindex/><h1 className="text-3xl font-black">{L.title}</h1><p className="mt-2 text-slate-400">{user.email}</p><form onSubmit={save} className="card mt-7 grid gap-4 p-6 md:grid-cols-2">
  <div className="md:col-span-2 flex flex-wrap items-center gap-4 rounded-xl bg-white/5 p-4">{form.avatar_url?<img src={form.avatar_url} alt={L.photo} className="size-20 rounded-2xl object-cover"/>:<div className="grid size-20 place-items-center rounded-2xl bg-slate-800 text-2xl font-black">{(form.display_name||form.full_name||user.email||'?').slice(0,1).toUpperCase()}</div>}<div><p className="font-semibold">{L.photo}</p><p className="mt-1 text-xs text-slate-500">{L.photoHint}</p><label className="btn-ghost mt-2 cursor-pointer">{uploading?L.uploading:L.choose}<input aria-label={L.choose} className="hidden" type="file" accept="image/jpeg,image/png,image/webp" disabled={uploading} onChange={e=>uploadAvatar(e.target.files?.[0])}/></label></div></div>
  ...
  <div className="md:col-span-2 flex flex-wrap items-center gap-3"><button className="btn-primary">{L.save}</button><Link className="btn-ghost" to={pathFor('/auth')+'?mode=forgot'}>{L.reset}</Link><span className="rounded-full bg-white/5 px-3 py-2 text-xs text-slate-400">{L.language}: {lang}</span>{status&&<span role="status" aria-live="polite" className="text-sm text-emerald-300">{status}</span>}</div>
 </form></section>
}
