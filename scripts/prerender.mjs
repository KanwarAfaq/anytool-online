...
await writeFile(resolve(root,'dist','tool-catalog.json'),JSON.stringify({
 ...
 tools:tools.map(t=>({
  ...
  processing:['image-resize','image-compress','png-to-jpg','jpg-to-png','dpi-calculator','qr-generator','qr-scanner','pdf-merge','pdf-split','percentage','loan-payment','random-picker','timer','image-to-sketch'].includes(t.slug)?'browser-local':t.category==='ai'?'authenticated-ai':'deterministic-web',
