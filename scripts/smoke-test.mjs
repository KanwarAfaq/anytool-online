...
assert.equal(tools.length,30,'expected 30 public tools')
assert.equal(new Set(tools.map(t=>t.slug)).size,tools.length,'tool slugs must be unique')
for(const slug of ['random-picker','timer','image-to-sketch','simple-calculator','love-calculator','zodiac-matcher','image-flip']) assert.ok(tools.some(t=>t.slug===slug),'missing public tool '+slug)
