...
test('new picker wheel accepts editable entries and produces a selection', async ({ page }) => {
  ...
  await expect(page.getByLabel('Spinner sound').locator('option')).toHaveCount(5)
  await page.getByLabel('Spinner sound').selectOption('silent')
  await page.getByLabel('Spin speed').selectOption('quick')
...
test('simple calculator performs arithmetic percent sign and keyboard input', async ({ page }) => {
...
test('love calculator gives deterministic playful result for two names', async ({ page }) => {
  await page.goto('/tools/love-calculator')
  await page.getByLabel('First name').fill('David')
  await page.getByLabel('Second name').fill('Daisy')
  await page.getByRole('button',{name:'Calculate love'}).click()
  const score=page.locator('.love-score-ring strong')
  await expect(score).toContainText('%')
  const first=await score.textContent()
  await page.reload()
  await page.getByLabel('First name').fill('Daisy')
  await page.getByLabel('Second name').fill('David')
  await page.getByRole('button',{name:'Calculate love'}).click()
  await expect(page.locator('.love-score-ring strong')).toHaveText(first)
})

test('zodiac matcher updates compatibility and can swap signs', async ({ page }) => {
  await page.goto('/tools/zodiac-matcher')
  await page.getByLabel('First zodiac sign').selectOption('Aries')
  await page.getByLabel('Second zodiac sign').selectOption('Libra')
  const score=page.locator('.zodiac-score strong')
  await expect(score).toContainText('%')
  const before=await score.textContent()
  await page.getByRole('button',{name:'Swap zodiac signs'}).click()
  await expect(page.getByLabel('First zodiac sign')).toHaveValue('Libra')
  await expect(page.getByLabel('Second zodiac sign')).toHaveValue('Aries')
  await expect(score).toHaveText(before)
})

test('image flip toggles horizontal and vertical states locally', async ({ page }) => {
  const png=Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Wl8V1kAAAAASUVORK5CYII=','base64')
  await page.goto('/tools/image-flip')
  await page.getByLabel('Choose image to flip').setInputFiles({name:'one.png',mimeType:'image/png',buffer:png})
  const canvas=page.locator('canvas[aria-label="Flipped image preview"]')
  await expect(page.getByRole('button',{name:/Flip left/})).toBeEnabled()
  await page.getByRole('button',{name:/Flip left/}).click()
  await expect(canvas).toHaveAttribute('data-flip-x','true')
  await page.getByRole('button',{name:/Flip top/}).click()
  await expect(canvas).toHaveAttribute('data-flip-y','true')
